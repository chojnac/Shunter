...
extension FeatureManager {
    ...
    public func setupShakeHandler(enable: Bool) {
        ...
        #if canImport(UIKit)
        NotificationCenter.default.addObserver(forName: .FeatureManagerShakeNotification,
                                               object: nil,
...
private final class RootFeatureOverrideController: UINavigationController {
    ...
    convenience init(featureManager: FeatureManager) {
        ...
        viewController.navigationItem.leftBarButtonItem = UIBarButtonItem(title: NSLocalizedString("FeatureOverride.Button.Close", comment:""), style: .done, target: self, action: #selector(dismissOverrideScreen))
    }
